...
async function querySingle(query){
...
async function queryAllNoPagination(query) {
    return new Promise((resolve, reject) => {
        let db = new sqlite3.Database('./services/exercises.db', sqlite3.OPEN_READONLY, (err) => {
            if (err) {
                console.error(err.message);
...
async function queryAll(query, page = 1, pageSize = 15) {
...
module.exports = { queryAll,querySingle,queryAllNoPagination }